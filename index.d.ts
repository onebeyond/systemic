...
type MissingDependencies<
  ...
  TNames extends unknown[]
> = TNames extends [infer Name, ...infer Rest]
...
type SimpleDependsOnOption<TDependencyKeys, TSystemic> = TDependencyKeys &
  keyof TSystemic
type MappingDependsOnOption<TDependencyKeys, TSystemic> =
  TDependencyKeys extends keyof TSystemic
    ? {
...
type DependsOnOption<TDependencyKeys, TSystemic> =
  | SimpleDependsOnOption<TDependencyKeys, TSystemic>
  | MappingDependsOnOption<TDependencyKeys, TSystemic>

type DependsOn<
  TSystemic extends Record<string, unknown>,
  TDependencies extends Record<string, unknown>
> = {
  /**
...
export type Systemic<T extends Record<string, unknown>> = {
   ...
   */
  merge: <TSubSystem extends Record<string, unknown>>(
    subSystem: Systemic<TSubSystem>
  ) => Systemic<T & TSubSystem>

  /**
   * Includes a subsystem into this systemic system
   */
  include: <TSubSystem extends Record<string, unknown>>(
    subSystem: Systemic<TSubSystem>
  ) => Systemic<T & TSubSystem>
   ...
   */
  bootstrap: <
    TSystem extends Record<string, unknown> = Record<string, unknown>
  >(
    path: string
  ) => Systemic<TSystem>
}
